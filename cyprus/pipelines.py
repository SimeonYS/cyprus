...
class CyprusPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cyprus`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cyprus WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cyprus (date, title, link, content)"
